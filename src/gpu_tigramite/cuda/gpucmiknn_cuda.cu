...
#define BIG_DOUBLE 1.7976931348623157e308 // DBL_MAX equivalent for double precision
...
__global__ void
pipeline_mi_calculation_l0_runge(double* obs_dev, int* block_starts, int* tail_indices, double* mi_values, size_t obs_count, size_t dim, int k, int n_blks, size_t sig_blocklength, size_t has_tail, size_t tail_length){
    ...
    double k_distances[MAX_K_RUNGE];
    ...
    for(int b = 0; b < obs_count; b+=THREAD_BLOCK_SIZE){
        ...
        for(int t = 0; t < THREAD_BLOCK_SIZE; t++){
            ...
            // Skip self-comparison (distance to self would be 0)
            if(obs_id_a == b + t){
                continue;
            }

            double tmp_chebyshev = max(abs(shared_mem[X_a_start_index + t_x] - shared_mem[X_b_start_index + t]), abs(shared_mem[Y_a_start_index + t_x] - shared_mem[Y_b_start_index + t]));
    ...
    // Apply epsilon reduction to enforce strict inequality (matches CPU Tigramite behavior)
    double kth_dist = k_distances[k-1] * 0.999999999;
    if (kth_dist < 1e-20) kth_dist = 1e-20;
    ...
    for(int b = 0; b < obs_count; b+=THREAD_BLOCK_SIZE){
        ...
        for(int t = 0; t < THREAD_BLOCK_SIZE; t++){
            ...
            double tmp_chebyshev_x = abs(shared_mem[X_a_start_index + t_x] - shared_mem[X_b_start_index + t]);
            nx_counter += (tmp_chebyshev_x < kth_dist);

            double tmp_chebyshev_y = abs(shared_mem[Y_a_start_index + t_x] - shared_mem[Y_b_start_index + t]);
            ny_counter += (tmp_chebyshev_y < kth_dist);
        }
    }
    if (nx_counter < 1) nx_counter = 1;
    if (ny_counter < 1) ny_counter = 1;
    double partial_mi_value = digamma(nx_counter) + digamma(ny_counter);
...
__global__ void
pipeline_mi_calculation_l0_row_runge(double* obs_dev, int* block_starts, int* tail_indices, double* mi_values, size_t obs_count, int k, size_t n_blks, size_t sig_blocklength, int has_tail, size_t tail_length, int x_id, int* y_candidates){
    extern __shared__ double shared_mem[];
    __shared__ int tail_index;
    __shared__ int X_a_start_index;
    __shared__ int Y_a_start_index;
...
double pval_l0_cuda_shared(const double*  obs,
                          size_t        obs_count,
                          size_t        dim,
                          int           k,
                          size_t        permutations,
                          double*        mi_value)
{
    ...
    const unsigned int size_of_double = sizeof(double);
    const unsigned int size_of_int = sizeof(int);
    cudaSetDevice(0);

    // Load Data to GPU
    double* obs_dev = NULL;
    cudaMalloc(&obs_dev, obs_count * dim * size_of_double);
    cudaMemcpy(obs_dev, obs, obs_count * dim * size_of_double, cudaMemcpyHostToDevice);
    
    double* mi_values =  NULL;
    cudaMallocManaged(&mi_values, (permutations+1) * size_of_double);
    for(int i = 0; i < (permutations + 1); i++){
        mi_values[i] = 0;  // Fixed: Initialize to 0, not log(obs_count)
    }
    ...
    // Use deterministic seeding (fixed seed) for reproducibility
    std::mt19937 g(42);

    for(int i = 0; i < permutations + 1; i++){
        if(i > 0){
            // Seed with iteration index to ensure different but deterministic permutations
            g.seed(42 + i);
            std::shuffle(block_starts+(i*n_blks), block_starts+((i+1) * n_blks), g);
            if(has_tail){
                tail_indices[i] = block_starts[g() % n_blks];
            }else{
    ...
    pipeline_mi_calculation_l0_runge<<<grid0, block0, (4 * THREAD_BLOCK_SIZE) * size_of_double>>>(obs_dev, block_starts, tail_indices, mi_values, obs_count, dim, k, n_blks, sig_blocklength, has_tail, tail_length);
    cudaDeviceSynchronize();

    // Calculate PValue and MI
    double div = obs_count * 1.0;
    double base_mi = digamma(k) + digamma(obs_count) - mi_values[0]/div;
    double mi = 0.0;
    double pvalue = 0.0;
...
__global__ void
pipeline_mi_calculation_ln_runge(double* obs_yz_dev, double* x_permutations_dev, double* mi_values, size_t obs_count, size_t dim, int k, int k_perm){
    extern __shared__ double shared_mem[];
    double xyz_k_distances[MAX_K_RUNGE];
    ...
    // Apply epsilon reduction to enforce strict inequality (matches CPU Tigramite behavior)
    double xyz_kth_dist = xyz_k_distances[k - 1] * 0.999999999;
    if (xyz_kth_dist < 1e-20) xyz_kth_dist = 1e-20;
    ...
    if (nxz_counter < 1) nxz_counter = 1;
    if (nyz_counter < 1) nyz_counter = 1;
    if (nz_counter < 1) nz_counter = 1;
    double partial_mi_value = (digamma(nxz_counter) + digamma(nyz_counter) - digamma(nz_counter));
...
__global__ void
pipeline_mi_calculation_ln_row_runge(double* obs_dev, double* x_permutations_dev, double* mi_values, size_t obs_count, int dim, int k, int k_perm, int x_id, size_t permutations, int* sList_dev, size_t sEntries, size_t varCount, int* yCandidates, size_t yDim, int* yPosition, int* oldSEntryPointer, int* newSEntryPointer){
    extern __shared__ double shared_mem[];
    double xyz_k_distances[MAX_K_RUNGE];
...
__global__ void
pipeline_rperm_ln_multi(double* obs_z, int* perm_x, int* used, size_t obs_count, size_t dim, size_t permutations){
    ...
    extern __shared__ double shared_mem[];
    double z_k_distances[SHUFFLE_NEIGHBORS];
...
__global__ void
pipeline_rperm_ln_multi_all(double* obs, double* perm_x, int* used, size_t obs_count, size_t dim, size_t permutations, int* sList_dev, size_t sEntries, int x_id){
    // random order going through X (could this be achieved through scheduling of thread blocks ?)
    // neighbors for each X_i of size count == 5 / 10 --> depends on variable shuffle_neighbors (reuse neighbor calc)
    // calculate shuffled neighbors
    // access use index through atomiccas auf position in neighbors (yes we go through it ordered within the local list, but do not guarantee that each thread is executed in order --> different used)
...
void perm_cuda_multi(const double* obs, size_t obs_count, size_t dim, size_t permutations, int* x_permutations){
    cudaProfilerStart();
    const int BLOCKS_PER_PERMUTATION = (obs_count + THREAD_BLOCK_SIZE_SHUFFLE - 1) / THREAD_BLOCK_SIZE_SHUFFLE;
    const unsigned int size_of_double = sizeof(double);
    cudaSetDevice(0);
    // Load Data to GPU
    double* obs_z_dev = NULL;
    int* perm_x_dev = NULL;
    int* used = NULL;
    cudaMalloc(&obs_z_dev, obs_count * dim * size_of_double);
    cudaMemcpy(obs_z_dev, obs, obs_count * dim * size_of_double, cudaMemcpyHostToDevice);
    ...
    pipeline_rperm_ln_multi<<<grid0, block0, THREAD_BLOCK_SIZE_SHUFFLE * dim * size_of_double>>>(obs_z_dev, perm_x_dev, used, obs_count, dim, permutations);
    cudaDeviceSynchronize();
...
void perm_cuda_multi_all(const double* obs, size_t obs_count, size_t vars, size_t permutations, double* x_permutations,
                        int* sList_c, size_t dim, size_t sEntries, int x_id)
{
    ...
    double* obs_dev = NULL;
    double* perm_dev = NULL;
    int* sList_dev = NULL;
    int* used = NULL;
    cudaMalloc(&obs_dev, obs_count * vars * size_of_double);
    cudaMemcpy(obs_dev, obs, obs_count * vars * size_of_double, cudaMemcpyHostToDevice);
    cudaMalloc(&perm_dev, obs_count * permutations * sEntries * size_of_double);
    ...
    cudaMemset(perm_dev, 0, obs_count * permutations * sEntries * size_of_double);
    ...
    dim3 block0(THREAD_BLOCK_SIZE_SHUFFLE, 1, 1);
    pipeline_rperm_ln_multi_all<<<grid0, block0, THREAD_BLOCK_SIZE_SHUFFLE * dim * size_of_double>>>(obs_dev,
        perm_dev, used, obs_count, dim, permutations, sList_dev, sEntries, x_id);
    cudaDeviceSynchronize();
    cudaMemcpy(x_permutations, perm_dev, obs_count * permutations * sEntries * size_of_double, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    // Free Resources
    cudaFree(obs_dev);
    cudaFree(perm_dev);
...
void pval_l0_row_cuda(const double* data_c,
                      size_t obs_count,
                      int k,
                      size_t permutations,
                      int x_id,
                      ...
                      size_t yDim)
{
    ...
    double* obs_dev = NULL;
    cudaMalloc(&obs_dev, obs_count * varCount * size_of_double);
    cudaMemcpy(obs_dev, data_c, obs_count * varCount * size_of_double, cudaMemcpyHostToDevice);

    // Load tmpval structure for MI values to GPU
    double* mi_values =  NULL;
    cudaMallocManaged(&mi_values, (permutations+1) * yDim * size_of_double);
    memset(mi_values, 0, (permutations+1) * yDim * size_of_double);
    ...
    for (size_t y = 0; y < yDim; ++y) {
        for(size_t i = 0; i < permutations + 1; i++){
            if(i > 0){
                // Seed based on y and i to ensure batch consistency
                // If we run sequential, y is always 0.
                // If we run batch, y varies.
                // To match sequential and batch, the permutation for y=0 in batch must match y=0 in sequential.
                // So seed should depend on y.
                g.seed(42 + y * 10000 + i);
                std::shuffle(block_starts+((y * n_blks * (permutations + 1)) + (i*n_blks)),
                             block_starts+((y * n_blks * (permutations + 1)) + ((i+1) * n_blks)), g);
                if(has_tail){
                    tail_indices[(y * (permutations + 1)) + i] = block_starts[(y * n_blks * (permutations + 1)) + g() % n_blks];
                }else{
    ...
    double pvalue, base_mi, mi, div;
    // Pipelined MI Calculation
    dim3 grid0(yDim, permutations + 1, BLOCKS_PER_PERMUTATION);
    dim3 block0(THREAD_BLOCK_SIZE, 1, 1);
    pipeline_mi_calculation_l0_row_runge<<<grid0, block0, (4 * THREAD_BLOCK_SIZE) * size_of_double>>>(obs_dev, block_starts, tail_indices, mi_values, obs_count, k, n_blks, sig_blocklength, has_tail, tail_length, x_id, y_candidates_dev);
    div = obs_count * 1.0;
    cudaDeviceSynchronize();
...
void pval_ln_row_cuda(const double* obs, const double* x_permutations_c, size_t obs_count, int k, int k_perm, size_t permutations, int x_id, size_t varCount, int lvl, int* sList_c, size_t sEntries, int* sOfX, double* pvalOfX, double alpha, int* candidates_c, size_t* yDim, size_t originalYDim, bool splitted) {
    ...
    double* obs_dev = NULL;
    cudaMalloc(&obs_dev, obs_count * varCount * size_of_double);
    cudaMemcpy(obs_dev, obs, obs_count * varCount * size_of_double, cudaMemcpyHostToDevice);
    ...
    double* x_permutations_dev = NULL;
    cudaMalloc(&x_permutations_dev, obs_count * permutations * sEntries * size_of_double);
    cudaMemcpy(x_permutations_dev, x_permutations_c, obs_count * permutations * sEntries * size_of_double, cudaMemcpyHostToDevice);
    ...
    while (counter > 0 && remainingY > 0) {
        ...
        dim3 block0(THREAD_BLOCK_SIZE, 1, 1);
        pipeline_mi_calculation_ln_row_runge<<<grid0, block0, 2 * THREAD_BLOCK_SIZE * (lvl + 2) * size_of_double>>>(obs_dev, x_permutations_dev, mi_values, obs_count, (lvl + 2), k, k_perm, x_id, permutations, sList_dev, sEntries, varCount, y_candidates_dev, *yDim, yPosition, oldSEntryPointer, newSEntryPointer);
        cudaDeviceSynchronize();
        double pvalue, base_mi, cmi;
        int yPos = 0;
...
double pval_ln_cuda(const double* obs, const double* x_permutations, size_t obs_count, size_t dim, int k, int k_perm, size_t permutations, double* mi_value){
    // Constants
    const int BLOCKS_PER_PERMUTATION = (obs_count + THREAD_BLOCK_SIZE - 1) / THREAD_BLOCK_SIZE;
    const unsigned int size_of_double = sizeof(double);
    cudaSetDevice(0);
    ...
    // Load Data to GPU
    double* obs_yz_dev = NULL;
    cudaMalloc(&obs_yz_dev, obs_count * (dim - 1) * size_of_double);
    cudaMemcpy(obs_yz_dev, obs + obs_count, obs_count * (dim - 1) * size_of_double, cudaMemcpyHostToDevice);
    ...
    double* x_permutations_dev = NULL;
    cudaMalloc(&x_permutations_dev, obs_count * (permutations + 1) * size_of_double);
    cudaMemcpy(x_permutations_dev, obs, obs_count * size_of_double, cudaMemcpyHostToDevice);
    cudaMemcpy(x_permutations_dev + obs_count, x_permutations, obs_count * permutations * size_of_double, cudaMemcpyHostToDevice);

    double pvalue = 0.0;
    // Pipelined MI Calculation
    dim3 grid0(permutations + 1, BLOCKS_PER_PERMUTATION, 1);
    dim3 block0(THREAD_BLOCK_SIZE, 1, 1);
    pipeline_mi_calculation_ln_runge<<<grid0, block0, THREAD_BLOCK_SIZE * dim * size_of_double>>>(obs_yz_dev, x_permutations_dev, mi_values, obs_count, dim, k, k_perm);
    cudaDeviceSynchronize();
    double base_mi = digamma(k) - mi_values[0]/obs_count;
    for(int i = 1; i < permutations+1; i++){
